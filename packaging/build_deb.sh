...
# Usage: bash packaging/build_deb.sh  ->  lessan-ai_1.0.0-1_amd64.deb
set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
PKG_DIR="$ROOT_DIR/packaging"
PAYLOAD_DIR="$PKG_DIR/opt/lessan-ai"
DEB_NAME="lessan-ai_1.0.0-1_amd64.deb"
...
# --- 1. Stage project source into payload (unless already staged) ---
if [ ! -f "$PAYLOAD_DIR/main.py" ]; then
...
echo "1.0.0-1" > "$PAYLOAD_DIR/VERSION"
# --- 2. Icon: rasterize the SVG to PNG sizes ----------------------
