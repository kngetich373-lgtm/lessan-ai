"""Model Router — provider-agnostic AI model routing for Lessan AI.

The Model Router is the "Models" domain of the architecture. It selects the
best AI provider/model for every request while remaining completely
independent of any specific AI vendor.

Public API
----------
- :class:`ModelRouter` — the router itself (implemented by
  :mod:`core.model_router.router`). It also implements the orchestrator's
  ``ModelRouter`` interface.
- :class:`BaseModelProvider` — interface every AI provider adapter must
  implement.
- :class:`ProviderRegistry` — registration/lookup of providers.
- :class:`RoutingStrategy` — scoring/selection of the best route.
- :class:`FallbackStrategy` — ordered fallback when the primary route fails.
- :class:`ProviderHealthMonitor` — periodic provider health probing.

Internals
---------
- :mod:`core.model_router.models` — provider-neutral value objects.
- :mod:`core.model_router.di` — dependency-injection wiring.
"""
...
from core.model_router.capabilities import (
    ...
    CAPABILITY_BACKEND_DEV,
    CAPABILITY_CODE_REVIEW,
    CAPABILITY_DATA_ANALYSIS,
    CAPABILITY_DATABASE,
    CAPABILITY_DEBUGGING,
    CAPABILITY_DEVOPS,
    CAPABILITY_DOCUMENTATION,
    CAPABILITY_FRONTEND_DEV,
    CAPABILITY_FULLSTACK_DEV,
    CAPABILITY_GENERAL_CHAT,
    CAPABILITY_LONG_CONTEXT,
    CAPABILITY_MACHINE_LEARNING,
    CAPABILITY_MOBILE_DEV,
    CAPABILITY_PERFORMANCE,
    CAPABILITY_REASONING,
    CAPABILITY_SECURITY,
    CAPABILITY_TESTING,
    CAPABILITY_WEB_SCRAPING,
)
...
# Event topics published on the event bus by the router.
from core.model_router.router import (
    EV_PROVIDER_HEALTH_CHANGED,
    ...
    EV_ROUTE_REQUESTED,
    EV_ROUTE_SELECTED,
    EV_ROUTE_STREAM_CHUNK,
    EV_ROUTE_STREAM_COMPLETED,
    EV_ROUTE_STREAM_STARTED,
    EV_ROUTE_SUCCEEDED,
)
