...
# ── X11 / Wayland display fix ────────────────────────────────────────
# pyautogui needs DISPLAY + XAUTHORITY to talk to the X server.
# If they are missing (cron, headless, systemd …) fall back to :0.
if "DISPLAY" not in os.environ or not os.environ["DISPLAY"]:
    os.environ["DISPLAY"] = ":0"

# Mutter's Xwayland auth cookies are unreadable by python3-Xlib's
# bundled Xauth parser.  Grant local X access via xhost instead.
_xhost_user = os.environ.get("USER") or os.environ.get("LOGNAME")
if _xhost_user:
    try:
        subprocess.run(
            ["xhost", f"+SI:localuser:{_xhost_user}"],
