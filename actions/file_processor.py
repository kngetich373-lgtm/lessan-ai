"""
file_processor.py — LESSAN Universal File Processor
...
class _Result:
    """Mimics genai response: supports .text and .strip()/.startswith()."""
...
class _OmniCompatModel:
    """Drop-in replacement for genai.GenerativeModel, backed by OmniRoute.

    Supports:
      - generate_content(str)                       → text chat
      - generate_content([prompt, PIL.Image])       → vision (via OmniRoute)
      - generate_content([prompt, {"mime_type", "data"}]) → vision/audio best-effort
    """
    def __init__(self, model_name: str = "gemini-2.5-flash",
                 system_instruction=None):
        self.model_name = model_name
        self.system     = system_instruction or (
            "You are a helpful file-processing assistant. Be accurate and concise."
        )
    ...
    def generate_content(self, content):
        ...
        if isinstance(content, list):
            ...
            for part in content:
                if isinstance(part, str):
                    ...
                    tmp  = Path(tempfile.mktemp(
                        suffix=".png" if not mime.startswith("audio")
                        else ".mp3"
                    ))
            ...
            prompt = "\n".join(prompt_parts) or "Describe this."
            try:
                if image_path:
                    try:
                        text = client.vision_from_file(
                            prompt, image_path, system=self.system
                        )
                    finally:
                        try:
                ...
                if audio_data:
                    return _Result(
                        "Audio transcription requires the Gemini Live API. "
                        "OmniRoute free text models cannot process raw audio — "
                        "please transcribe to text first."
                    )
                return _Result(client.chat(prompt, system=self.system))
            finally:
                if audio_data:
                    try:
                        Path(audio_data).unlink()
                    except Exception:
                        pass
        return _Result(client.chat(str(content), system=self.system))


def _gemini_client():
    return _OmniCompatModel("gemini-2.5-flash")
