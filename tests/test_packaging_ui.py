...
def test_packaging_stages_documents_runtime_package():
    script = (ROOT / "packaging" / "build_deb.sh").read_text(encoding="utf-8")
    assert "documents" in script
    assert "documents/action.py" in script
...
def test_runtime_entrypoint_uses_redesigned_ui():
    ...
    assert "from documents.action import generate_document, list_document_types" in main
